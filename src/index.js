...
import hoistVariablesModule from "@babel/helper-hoist-variables";
import { template, types as t } from "@babel/core";
import { buildDynamicImport, getModuleName, rewriteThis, } from "@babel/helper-module-transforms";
import { isIdentifierName } from "@babel/helper-validator-identifier";

const hoistVariables = hoistVariablesModule.default;
...
function constructExportCall(path, exportIdent, exportNames, exportValues, exportStarTarget, stringSpecifiers) {
  ...
  if (!exportStarTarget) {
    if (exportNames.length === 1) {
      statements.push(t.expressionStatement(t.callExpression(exportIdent, [
      ...
      for (let i = 0; i < exportNames.length; i++) {
        ...
        objectProperties.push(t.objectProperty(stringSpecifiers.has(exportName)
          ? t.stringLiteral(exportName)
          : t.identifier(exportName), exportValue));
      }
...
export default declare((api, options) => {
  ...
  const reassignmentVisitor = {
    "AssignmentExpression|UpdateExpression"(path) {
      if (reassignmentVisited.has(path.node))
        return;
      ...
      if (isPostUpdateExpression) {
        node = t.binaryExpression(
          ...
          node.operator[0], t.unaryExpression("+", t.cloneNode(
            // @ts-expect-error node is UpdateExpression
            node.argument)), t.numericLiteral(1));
      }
      for (const exportedName of exportedNames) {
  ...
  return {
    name: "transform-modules-commonjs-to-systemjs",
    pre() {
      ...
      Program: {
        ...
        exit(path, state) {
          ...
          function buildExportCall(name, val) {
            return t.expressionStatement(t.callExpression(t.identifier(exportIdent), [
